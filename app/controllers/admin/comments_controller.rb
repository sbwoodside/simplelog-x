...
class Admin::CommentsController < Admin::BaseController
  ...
  def comments_by_post
    ...
    # grab the comments (join on posts for titles)
    @comments = Comment.find(:all, :select => 'comments.*, posts.title',
      :joins => 'left outer join posts on comments.post_id = posts.id',
      :conditions => ['post_id = ?', params[:id]], :order => @sorter.to_sql)
    $admin_page_title = 'Listing comments'
    render :template => 'admin/comments/comment_list'
  end
